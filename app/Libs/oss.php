...
//账号
    ...
    public function CookieSet( $cookie , $time )
    {
        $newConfig = '<?php 
      $config = array(
        "cookie" => "' . $cookie . '",
    ...
    public function upload( $file , $multipart = true , $cookie )
    {
        ...
        $ch = curl_init( $url );
        curl_setopt_array(
            $ch , array( CURLOPT_POST => true , CURLOPT_VERBOSE => true , CURLOPT_RETURNTRANSFER => true , CURLOPT_HTTPHEADER => array( "Cookie: $cookie" ) , CURLOPT_POSTFIELDS => $post , )
        );
        $output = curl_exec( $ch );
        curl_close( $ch );
        // 正则表达式提取返回结果中的json数据
